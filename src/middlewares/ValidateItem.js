...
const validarCadastroItem = (req, res, next) => {
    ...
    if (!nome || !preco || !icone) {
        return res.status(400).json({ success: false, message: MENSAGENS.ITEM.ERRO_CADASTRO });
    }
    next();
}
...
const validarCompraItem = async (req, res, next) => {
    ...
    if (!id || !email) {
        return res.status(400).json({ success: false, message: MENSAGENS.ITEM.ERRO_COMPRA });
    }

    if (await itemInexistente(id)) {
        return res.status(400).send({ success: false, message: MENSAGENS.ITEM.NAO_EXISTE });
    }
    ...
    if (await itemPossuido(id, email)) {
        return res.status(200).send({ success: false, message: MENSAGENS.ITEM.JA_POSSUIDO });
    }

    if (await pontosInsuficientes(id, email)) {
        return res.status(200).send({ success: false, message: MENSAGENS.ITEM.PONTOS_INSUFICIENTES });
    }
...
const validarIdItem = async (req, res, next) => {
    ...
    if (!id) {
        return res.status(400).json({ success: false, message: MENSAGENS.ITEM.ID_NAO_INFORMADO });
    }
...
const validarAtualizarItem = async (req, res, next) => {
    ...
    if (!nome && !preco && !icone) {
        return res.status(400).json({ success: false, message: MENSAGENS.ITEM.ERRO_ATUALIZAR });
    }
